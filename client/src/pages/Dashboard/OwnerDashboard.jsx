  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        // Set stats
        setStats({
          shopkeepers: shopkeepersRes.data.length,
          products: productsRes.data.length,
          sales: salesRes.data.length,
        });

        // Calculate totals
        calculateTotals(productsRes.data, salesRes.data);
    ...
    <div className="min-h-screen bg-gray-100">
      ...
      <div className="p-6 sm:p-10">
        ...
        <DataTable 
          ...
          columns={["Product", "Shopkeeper", "Quantity", "Revenue (TSh)", "Profit (TSh)", "Date"]}
          renderCell={(item, column) => {
            const product = productsList.find(p => p.id === item.productId);
            const shopkeeper = shopkeepersList.find(s => s.id === item.shopkeeperId);
            const revenue = product ? (product.sellPrice || 0) * item.quantity : 0;
            const cost = product ? (product.buyPrice || 0) * item.quantity : 0;
            const profit = revenue - cost;
            
            if (column === "Product") return product?.name || `Product ${item.productId}`;
            if (column === "Shopkeeper") {
              return shopkeeper ? `${shopkeeper.firstName} ${shopkeeper.lastName}` : `Shopkeeper ${item.shopkeeperId}`;
            }
            if (column === "Quantity") return item.quantity || "—";
            if (column === "Revenue (TSh)") return formatTZS(revenue);
            if (column === "Profit (TSh)") return (
              <span className={profit >= 0 ? "text-green-600 font-semibold" : "text-red-600 font-semibold"}>
                {formatTZS(profit)}
              </span>
            );
            if (column === "Date") {
              return new Date(item.createdAt).toLocaleDateString('sw-TZ');
            }
            return "—";
          }}
        />
...
const DataTable = ({ title, data, columns, renderCell }) => (
              ...
              <tr key={item.id || idx} className="hover:bg-gray-50">
                {columns.map(column => (
                  <td key={`${item.id || idx}-${column}`} className="px-4 py-2 border">
                    {renderCell ? renderCell(item, column) : String(item[column] || item[column.toLowerCase()] || "—")}
                  </td>
                ))}
              </tr>
            ))}
